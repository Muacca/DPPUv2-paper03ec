...
\midrule
EC local min.\ on $\eta=V=0$ ($\alpha<0$) & absent & \textbf{present (new)} & absent \\
...
We establish the scaling exponent of the $\Nilthree\times\Sone$ EC false vacuum
analytically.
...
\begin{figure}[H]
  ...
  \caption{$\gamma=1/2$ scaling: $r_0\propto\sqrt{|\alpha|}$ for the
    $\Nilthree$ EC false vacuum.  Analytic predictions (line) agree with
...
\subsection{Conformal Flatness and EC Local Minima on the $\eta=V=0$ Section}
\label{sec:compare:conformal}

We state a unifying theorem for the appearance of EC vacua in all three
topologies.

\begin{theorem}[Conformal flatness and EC local minima]\label{thm:confflat}
  \begin{equation}
    C^2_\LC(\eta=V=0) = 0
    \;\Longrightarrow\;
    \text{no EC local minimum appears on the }\eta=V=0\text{ section.}
  \end{equation}
  \begin{itemize}
    \item $\Tthree$ (flat): $C^2_\LC=0$, so $\alpha$ does not modify
          $\Veff(\eta=V=0)$ and no EC local minimum appears.
    \item $\Sthree$ (conformally flat): $C^2_\LC(\Sthree,\eta=V=0)=0$, so
          likewise no EC local minimum appears.
    \item $\Nilthree$ (non-conformally flat): $C^2_\LC(\Nilthree)=4/(3R^4)\neq 0$,
          so $\alpha\cdot C^2_\LC\cdot\Vol = -64\pi^4 L\alpha/(3r)$ is added to
          the effective potential, and for $\alpha<0$ a false vacuum appears
          (Theorem~\ref{thm:fv}).
  \end{itemize}
  Among the three topologies studied, EC local minima on the $\eta=V=0$ section
...
$\eta=V=0$ section ($\Nilthree$, $\alpha<0$) & non-conf.\ flat LC background $\Rightarrow$ false vacuum & --- \\
\bottomrule
\end{tabular}%
}
