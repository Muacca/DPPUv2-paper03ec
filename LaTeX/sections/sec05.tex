...
this property generates---via the EC--Weyl coupling---an EC slice minimum
...
even on the $\eta=V=0$ section, generating the EC slice minimum
(\S\ref{sec:nil3:ecslice}).
...
\subsection{Theorem~6: $\Nilthree$ EC Slice Minimum}
\label{sec:nil3:ecslice}

\begin{theorem}[$\Nilthree$ EC slice minimum]\label{thm:ecslice}
  For $\alpha<0$, the homogeneous effective potential of $\Nilthree\times\Sone$
  restricted to the $\eta=V=0$ section supports a positive EC-induced slice
  minimum\textup{:}
  \begin{equation}
    ...
    \quad (\alpha = -a^2,\; a>0).
  \end{equation}
  Moreover, the stationary point $(r,\eta,V)=(r_0,0,0)$ is a local minimum of
  the full homogeneous potential for $|\kappa^2\theta_\NY|<1$, marginal for
  $|\kappa^2\theta_\NY|=1$, and a saddle for $|\kappa^2\theta_\NY|>1$.
\end{theorem}

\paragraph{Terminological note.}
Earlier drafts referred to this point as a ``false vacuum.''  In the present
revision we use ``EC slice minimum'' to emphasise that it is first identified
on the $\eta=V=0$ section and that full homogeneous stability requires the
explicit Hessian criterion stated above.
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \quad (\alpha = -a^2 < 0),
  \end{equation}
  establishing a positive slice minimum.

  Full homogeneous stability is decided by the Hessian at the stationary point:
  \begin{equation}
    H =
    \left.\frac{\partial^2\Veff}{\partial(r,\eta,V)^2}\right|_{(r_0,0,0)}
    =
    \begin{pmatrix}
      \dfrac{2\sqrt{3}\pi^4 L}{\kappa^3\sqrt{-\alpha}} & 0 & 0 \\[6pt]
      0 & \dfrac{128\sqrt{3}\pi^4 L\sqrt{-\alpha}}{\kappa}
        & -\dfrac{512\pi^4 L\alpha\kappa^2\theta_\NY}{3} \\[8pt]
      0 & -\dfrac{512\pi^4 L\alpha\kappa^2\theta_\NY}{3}
        & \dfrac{2048\sqrt{3}\pi^4 L\kappa(-\alpha)^{3/2}}{27}
    \end{pmatrix}.
  \end{equation}
  The $r$-direction principal minor is always positive, while the remaining
  $(\eta,V)$ block has determinant
  \begin{equation}
    \det H_{(\eta,V)}
    = \frac{262144\pi^8L^2\alpha^2}{9}
      \left(1-\kappa^4\theta_\NY^2\right).
  \end{equation}
  Sylvester's criterion therefore gives the stated condition
  $|\kappa^2\theta_\NY|<1$ for a full homogeneous local minimum, with the
  equality case marginal and the larger-$|\theta_\NY|$ regime a saddle.
  Script: \texttt{proofs/nil3\_slice\_minimum\_stability.py}.
\end{proof}
...
\bottomrule
\end{tabular}
\caption{Analytic vs.\ numeric comparison for the $\Nilthree$ EC slice minimum
  (with $\kappa=L=1$).}
\label{tab:nil3fv}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{$\Nilthree$ effective potential $\Veff(r)$ on the $\eta=V=0$ section
    ...
    creates a slice minimum at $r_0 = 2.309$ (solid).}
  \label{fig:nil3veff}
\end{figure}
...
\begin{theorem}[$\Nilthree$ quintet splitting]\label{thm:quintet}
  The spin-2 quintet (5 components) around the $\Nilthree\times\Sone$ EC
  slice-minimum branch splits as
  \begin{equation}
...
\bottomrule
\end{tabular}
\caption{$\Nilthree$ spin-2 quintet splitting at the EC slice minimum
...
\begin{figure}[H]
  ...
  \caption{$\Nilthree$ spin-2 quintet splitting $5\to 0+2+1+1$ at the EC slice
    minimum.  The zero mode $q_3$ is geometrically protected by the
...
\begin{theorem}[Uniaxial mass splitting]\label{thm:uniaxial}
  ...
  \end{equation}
  At the EC slice minimum $r_0 = 4\kappa a/\sqrt{3}$ ($\alpha=-a^2$)\textup{:}
  \begin{equation}
...
\subsection{EFT around the $\Nilthree$ EC Slice Minimum}
\label{sec:nil3:eft}

We determine the quadratic effective action around the EC slice minimum
$r_0=4\kappa a/\sqrt{3}$.  The quoted EFT below uses the standard benchmark
$\theta_\NY=0$, for which the spin-0 Hessian returns to a diagonal block.
...
\begin{tabular}{lccc}
\toprule
Quantity & AX ($V=0,\,\eta\neq 0$) & VT ($\eta=0,\,V\neq 0$) & EC slice min.\ ($\eta=V=0$, $\alpha<0$) \\
\midrule
...
root search by \texttt{paper03ec/nil3\_mx\_vacuum\_search.py} provides only a
finite sampled check for general $\theta_\NY$, and no MX stable point is found
within the dictionary range.  The rigorous statement used in this paper is
instead the analytic slice stationary point together with the full-Hessian
criterion $|\kappa^2\theta_\NY|<1$.  The EC-specific physics of $\Nilthree$
(EC slice minimum, quintet splitting) originates from $C^2_\LC\neq 0$ on the
$\eta=V=0$ section, independently of the MX correction.
