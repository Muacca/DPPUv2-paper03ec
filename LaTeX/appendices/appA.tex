...
$\Nilthree$ EFT at EC slice minimum ($\theta_\NY=0$ benchmark) & \texttt{paper03ec/nil3\_ec\_slice\_minimum\_eft.py} \\
$\Nilthree$ slice-minimum stability ($|\kappa^2\theta_\NY|<1$) & \texttt{proofs/nil3\_slice\_minimum\_stability.py} \\
...
\begin{enumerate}
  ...
  \item $\Nilthree$ slice stationary point: the full homogeneous Hessian is
        positive definite for $|\kappa^2\theta_\NY|<1$, marginal at equality,
        and a saddle above it (SymPy exact).  See
        \path{proofs/nil3_slice_minimum_stability.py}.
  \item $\Nilthree$ MX stationary-point search: for $\theta_\NY=0$ there is no
        real stationary point with $V\eta\neq 0$ in the $\alpha<0$ branch; for
        non-zero $\theta_\NY$, sampled root search finds no stable MX point
        within the dictionary range.
\end{enumerate}
