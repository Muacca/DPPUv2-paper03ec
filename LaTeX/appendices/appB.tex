...
\subsection{$\Nilthree\times\Sone$: EC False-Vacuum EFT}
\label{app:B:nil3}

\textbf{Setup:} $\Nilthree\times\Sone$, EC false vacuum
($\alpha=-a^2<0$, $\eta=V=0$, $r_0=4\kappa a/\sqrt{3}$).
``False vacuum'' refers to a local minimum of the homogeneous effective potential.

\paragraph{False vacuum identification.}
\begin{equation}
...
Scripts: \\
\texttt{paper03ec/nil3\_false\_vacuum\_eft.py},\texttt{paper03ec/nil3\_spin2\_quintet\_splitting.py}.
