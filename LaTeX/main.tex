...
\textbf{Theorem~6 ($\Nilthree$ EC slice minimum):}  For $\alpha<0$,
$\Nilthree\times\Sone$ supports a positive EC-induced slice minimum at
$r_0 = (4\kappa/\!\sqrt{3})\sqrt{|\alpha|}$ on the $\eta=V=0$ section; the
full homogeneous Hessian is positive definite for $|\kappa^2\theta_\NY|<1$,
marginal at equality, and a saddle above it.  The scaling exponent is
$\gamma=1/2$.

\textbf{Theorem~7 ($\Nilthree$ quintet splitting):}  The spin-2 quintet around
the EC slice-minimum branch splits as $5\to 0+2+1+1$; the zero mode $q_3$ is
...
\input{sections/sec02}   % EC-Weyl Coupling Structure
\input{sections/sec03}   % Mode Dictionary: S3 x S1
\input{sections/sec04}   % Mode Dictionary: T3 x S1
\input{sections/sec05}   % Mode Dictionary: Nil3 x S1
\input{sections/sec06}   % Three-Topology Comparison
\input{sections/sec07}   % Physical Interpretation and Outlook
...
%------------------------------------------------------------------------------
\appendix
\input{appendices/appA}  % Notation and Conventions
\input{appendices/appB}  % Complete EFT Coefficient List
\input{appendices/appC}  % Detailed Derivation of eps-s Cross-term
\input{appendices/appD}  % Auxiliary delta-sector Null Theorem
