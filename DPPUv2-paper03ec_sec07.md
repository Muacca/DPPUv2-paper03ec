...
**$Nil^3\times S^1$**: 非共形平坦多様体（ $C^2_{\rm LC} = 4/(3R^4) \neq 0$ ）であるため、EC-Weyl 結合が $\alpha < 0$ で false vacuum を生成する唯一のトポロジーである。この false vacuum は LC（ $\alpha=0$ ）には見えず、EC 拡張により初めて相構造に現れる。定理 6–8 が示す EC 固有の新物理（γ=1/2 スケーリング, quintet 分裂, 1 軸質量分裂）は、 $Nil^3$ の「中間的な非可換性」（ $T^3$ の全零と $S^3$ の三方向非零の中間）が、モード辞書の方向依存性として物理化した結果と理解できる。
...
**① Nil³ false vacuum（定理 6）**: $\alpha < 0$ での $Nil^3$ 安定真空は LC 世界では単調ポテンシャルしか持たない理論に、EC が「新しい安定相」を付け加える。 $r_0 \propto \sqrt{|\alpha|}$ のスケーリングは、Weyl coupling の強さが幾何的サイズを規定するという、一般相対論的には非自明な機構を示す。

**② quintet 分裂（定理 7）**: $Nil^3$ の 1 次元 Lie 代数構造（ $C^2_{01}$ のみ）が、EC false vacuum 周りで spin-2 の五重縮退を破り $5\to 0+2+1+1$ に分裂させる。特に zero mode $q_3$ の存在は Nil³ 固有の幾何的対称性の「残存」を示し、LC 等方点とは本質的に異なる物理的内容を持つ。
